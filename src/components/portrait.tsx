...
export function Portrait({ size = 'small', src = profile.profileImage }: { size?: 'small' | 'about'; src?: string }) {
  ...
  return <figure className={`portrait portrait-${size}`}>
    <div className="portrait-frame">{profile.portraitAvailable && !failed ?
      ...
      <img src={asset(src)} alt={t('Portrait of Triệu Phi')} width={600} height={750} onError={() => setFailed(true)} />
